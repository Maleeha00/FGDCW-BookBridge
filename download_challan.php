...
// Step 2: Import Dompdf classes
use Dompdf\Dompdf;
use Dompdf\Options;

// Step 3: Include your database config
include_once '../includes/config.php';

// Step 4: Fetch data (same as generate_challan.php)
...
$sql = "SELECT f.id as fine_id, f.amount, f.reason, b.book_name, u.name as student_name, u.unique_id as student_roll_no, u.department as student_class FROM fines f JOIN users u ON f.user_id = u.id JOIN issued_books ib ON f.issued_book_id = ib.id JOIN books b ON ib.book_id = b.id WHERE f.id = ? AND f.user_id = ?";
...
$amountInWords = numberToWords($data['amount']); // Assuming numberToWords function is in config.php or here

// Step 5: Create the HTML for the PDF
ob_start();
?>
<!DOCTYPE html>
<html>
    ...
    <div class="challan-container">
        <?php for ($i = 0; $i < 3; $i++): ?>
            ...
            <div class="challan-copy">
                <!-- (Yahan par challan ka poora HTML daalein jaisa generate_challan.php mein tha) -->
...
// Step 6: Instantiate and use the Dompdf class
$options = new Options();
