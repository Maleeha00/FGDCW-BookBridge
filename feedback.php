...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_feedback'])) {//check karta hai ke form POST request se submit hua ya nahi.
    $feedbackId = (int)$_POST['feedback_id'];//check karta hai ke delete button click hua ya nahi.
    
    $stmt = $conn->prepare("DELETE FROM feedback WHERE id = ?");//query safe banane ke liye ? placeholder use kiya.
...
$feedbacks = [];
while ($row = $result->fetch_assoc()) { //ek ek row associative array ki form me fetch karta hai.
    $feedbacks[] = $row; //sab rows ko ek array me daal diya.
}
?>
<h1 class="page-title">
